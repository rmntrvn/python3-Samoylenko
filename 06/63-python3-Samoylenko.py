...
for intf, attrib in fast_int['trunk'].items():
    print('interface FastEthernet' + intf)
    for command in trunk_template:
        if command.endswith('allowed vlan'):
            if attrib[0] == 'add':
                print(' {} {} {}'.format(command, attrib[0], ', '.join(attrib[1:])))
            if attrib[0] == 'only':
                print(' {} {}'.format(command, ', '.join(attrib[1::])))
            if attrib[0] == 'del':
                print(' {} {} {}'.format(command, 'remove', ', '.join(attrib[1:])))
        else:
            print(' {}'.format(command))